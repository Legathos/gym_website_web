const apiUrl = 'http://localhost:8080/';

export const EndpointDictionary = {
  addExercise:`${apiUrl}exercises/add-exercise`,
  getMonthlyWorkouts:`${apiUrl}workouts/get-monthly-workouts`,
  addFoodItem:`${apiUrl}food/add-food-item`,
  addNewLog:`${apiUrl}logger/add-new-log`,
  addSetTracking:`${apiUrl}set-tracking/add-set`,
  addWorkout:`${apiUrl}workouts/add-workout`,
  changePassword:`${apiUrl}user/change-password`,
  changeEmail:`${apiUrl}user/change-email`,
  checkIfUserExists:`${apiUrl}user/check-if-user-exists`,
  deleteExerciseById:`${apiUrl}exercises/delete-exercise-`,
  deleteFoodItem:`${apiUrl}delete-food-item-`,
  deleteLogItem:`${apiUrl}logger/delete-log`,
  deleteUser: `${apiUrl}user/delete-user-`,
  deleteWorkout:`${apiUrl}delete-workout-`,
  editFoodItem:`${apiUrl}edit-food-item`,
  editLogItem:`${apiUrl}logger/edit-log`,
  editUser:`${apiUrl}user/edit-user`,
  editWorkout:`${apiUrl}edit-workout`,
  estimateFoodFromImage:`http://localhost:5000/predict`,
  getAllExercises:`${apiUrl}exercises/get-all-exercises`,
  getExercisesByUserId:`${apiUrl}exercises/get-exercises-by-user-`,
  getAllFoodItems:`${apiUrl}food/get-food-items`,
  getExerciseById:`${apiUrl}exercises/get-exercise-`,
  getExercisesByCategory:`${apiUrl}get-exercises-by-`,
  getFoodItemByBarcodeFromApi:`${apiUrl}food/get-food-item-barcode-`,
  getFoodItemByBarcodeFromDatabase:`${apiUrl}food/get-food-item-by-barcode-`,
  getFoodItemById:`${apiUrl}get-food-item-`,
  getFoodItemsByName:`${apiUrl}food/get-food-items-`,
  getFoodTrackingByIdAndDate:`${apiUrl}logger/get-logs-for-`,
  getUserInfo:`${apiUrl}get-user-info-`,
  getUserInfoByUsername:`${apiUrl}user/get-user-username-`,
  getUserWeightHistory:`${apiUrl}user/weight-history-user-`,
  getWorkoutById:`${apiUrl}workouts/get-workout-`,
  getWorkoutByUserId:`${apiUrl}workouts/get-workouts-by-userid-`,
  getSetsByWorkout:`${apiUrl}set-tracking/get-sets-by-workout/`,
  getSetsByExercise:`${apiUrl}set-tracking/get-sets-by-exercise/`,
  getSetsByWorkoutAndExercise:`${apiUrl}workouts/get-set-by-workout-and-exercise/`,
  login:`${apiUrl}user/login`,
  register:`${apiUrl}user/register`,
  updateUserWeight:`${apiUrl}user/update-user-weight`,
};
